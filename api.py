...
def transcribe():
    ...
    if not url or not speaker_name:
        return jsonify({"message": "Error: URL and speaker_name are required."}), 400
    ...
    try:
        ...
        output_csv = f"{speaker_name}_metadata.csv"
        transcribe_audio_files(output_dir, output_csv)
        
        return jsonify({"message": "Transcription process completed successfully", "csv_file": output_csv}), 200
